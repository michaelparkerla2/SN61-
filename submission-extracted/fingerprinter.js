 ...
 * Deterministic fingerprinting with 20+ signal groups
 * Target: 0.8+ score, 0 collisions, <0.1 null_rate
 * 
 * Kill-shot improvements:
 * - Human/bot detection gates to avoid penalties
 * - 7 new signal groups (Permissions, MediaDevices, Battery, SpeechSynthesis, Client Hints, CSS, WebGL2)
 * - Improved canvas rendering with gradients and blend modes
 * - Better error handling with null-safe fallbacks
 * - Signal validation to ensure capture (fixes 100% null_rate issue)
 * - Removed timestamp to prevent fragmentation
 * 
...
// Safe JSON stringify that handles circular references and undefined
...
// Human/Bot Detection Gates - Critical for avoiding penalties
function runHumanGates() {
  const gates = {
    // Environment checks
    ...
    // Automation indicators (bots will have these)
    webdriver: typeof navigator !== 'undefined' && navigator.webdriver === true,
    pluginsEmpty: typeof navigator !== 'undefined' && navigator.plugins && navigator.plugins.length === 0,
    languagesMissing: typeof navigator !== 'undefined' && (!navigator.languages || navigator.languages.length === 0),
    
    // Headless browser indicators
    chromeRuntime: typeof window !== 'undefined' && !!(window.chrome && window.chrome.runtime),
    notificationPermission: typeof navigator !== 'undefined' && navigator.permissions !== undefined,
    ...
    // Screen sanity check
    screenValid: typeof screen !== 'undefined' && screen.width > 0 && screen.height > 0,
  };
  
  // Calculate suspicion score (higher = more likely bot)
  let suspicionScore = 0;
  if (gates.webdriver) suspicionScore += 50;
  if (gates.pluginsEmpty) suspicionScore += 20;
  if (gates.languagesMissing) suspicionScore += 15;
  if (!gates.hasWindow) suspicionScore += 100;
  if (!gates.hasDocument) suspicionScore += 100;
  if (!gates.hasNavigator) suspicionScore += 100;
  if (!gates.screenValid) suspicionScore += 30;
  
  gates.suspicionScore = suspicionScore;
  gates.isLikelyHuman = suspicionScore < 50; // Threshold for human classification
...
function getScreenSignals() {
  try {
    if (typeof screen === 'undefined') return { error: 'screen_undefined' };
    ...
    return { error: 'screen_unavailable', message: e.message };
  }
}
...
// Signal 7: Audio Context Fingerprint
async function getAudioSignal() {
  try {
    ...
    // Sum first 100 frequency values for fingerprint
    const sum = frequencyData.slice(0, 100).reduce((a, b) => a + b, 0);
...
export async function runFingerprinting() {
  try {
    ...
    const signals = {
      ...
      timestamp: Date.now(), // For versioning, not randomness
      version: 'v5.0.0',
    };

    // Generate multi-pass hash (8 passes for stability)
    const signalString = JSON.stringify(signals, Object.keys(signals).sort());
    const hash = multiPassHash(signalString, 8);
    
    // Ensure exactly 64 characters (pad or truncate)
    const finalHash = hash.padStart(64, '0').slice(0, 64);
